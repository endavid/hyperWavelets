...
public class ImageDisplay extends JComponent
	...
	public static final int ABSOLUTE = 1, RESCALE = 2, INVERSE = 3;
    ...
    private PlanarImage preprocess(PlanarImage im) {
       if (im.getSampleModel().getDataType() == DataBuffer.TYPE_BYTE) {
         return im; // it can be shown without problems
       } 
       ...
       switch (visualizationMode) {
       	case ABSOLUTE:
       		...
       		break;
   		case RESCALE:
   			prep=COps.rescaleSamples(im, 128f, 0.5f);
   			break;
   		case INVERSE:
    ...
    public synchronized void paintComponent(Graphics g) {
        ...
        for (tj = tymin; tj <= tymax; tj++) {
		    ...
		    if (sampleModel.getDataType() == DataBuffer.TYPE_BYTE) {
                      ...
                      // de momento esta es la solucion lenta para poder mostrar imagenes de enteros
  	    	      // Por ejemplo, valores de -255 a 255 los mapeamos en [0,255]
		      // Otra solucion seria aplicar antes otra operacion a la PlanarImage entera
			...
			/*	       
                      ...
                      for (int i=0;i<tile.getWidth();i++) {
                        for (int j=0;j<tile.getHeight();j++) {
                          for (int c=0;c<tile.getNumBands();c++) {
			    int v=tile.getSample(tile.getMinX()+i,tile.getMinY()+j,c);
			    //px[c]=(v+256)/2; // con esta conversion, sube brillo imagen media
                            px[c]=(Math.min(Math.abs(v), 255));
                       	  }
                          wr.setPixel(i,j,px);
                        }
                      }*/
		      
		      System.out.println("ImageDisplay: DataType not supported");
		    }
