...
public final class Panner extends JComponent
    ...
    private synchronized void initialize(JComponent item,
		...
		pb.addSource(image);
		pb.add(scale_x); pb.add(scale_y);
		pb.add(0f); pb.add(0f); pb.add(interp);
                temp = JAI.create("scale",pb);
            } else {
                temp = thumb;
            }
            ...
            if (slider == null) createSliderBox(tw, th);
        } else {
            if (slider == null) createSliderBox(32, 32);
        }
    ...
    public synchronized void set(JComponent jc,PlanarImage im, int maxSize) {
        initialize(jc,im,null,maxSize);
	repaint();
    }
